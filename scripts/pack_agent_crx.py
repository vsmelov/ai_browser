...
def main():
    if len(sys.argv) < 3:
        print("Usage: pack_agent_crx.py <agent_build_dir> <output_crx> [private_key.pem]", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("Error: Python package 'crx3' not found. Install it: pip install --user crx3", file=sys.stderr)
        sys.exit(1)
